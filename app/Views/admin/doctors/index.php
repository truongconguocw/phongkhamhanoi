...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div></div>
    <a href="/admin/doctors/create" class="btn btn-primary"><i class="fas fa-plus me-2"></i> Thêm Bác sĩ mới</a>
</div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php if (empty($doctors)): ?>
                        <tr>
                            <td colspan="4" class="text-center p-5 text-muted">Chưa có dữ liệu bác sĩ.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($doctors as $doctor): ?>
                            <tr>
                                <td>
                                    <div class="fw-bold"><?= htmlspecialchars($doctor['HoTen']) ?></div>
                                    <div class="small text-muted"><?= htmlspecialchars($doctor['Email']) ?></div>
                                </td>
                                <td><?= htmlspecialchars($doctor['TenChuyenKhoa'] ?? 'Chưa cập nhật') ?></td>
                                <td class="text-center">
                                    <?php if ($doctor['HoatDong']): ?>
                                        <span class="badge bg-success-soft text-success">Hoạt động</span>
                                    <?php else: ?>
                                        <span class="badge bg-danger-soft text-danger">Vô hiệu hóa</span>
                                    <?php endif; ?>
                                </td>
                                <td class="text-end">
                                    ...
                                    <form action="/admin/doctors/<?= $doctor['BacSiID'] ?>/delete" method="POST" class="d-inline">
...
<div id="confirmDeleteModal" class="confirm-modal">
    ...
    <div class="confirm-container">
        ...
        <div class="confirm-content">
            ...
            <p>Bạn có chắc chắn muốn xóa bác sĩ này? Hành động này không thể hoàn tác và có thể ảnh hưởng đến dữ liệu lịch hẹn, lịch làm việc liên quan.</p>
        </div>
...
<style>
    ...
    .bg-danger-soft { background-color: #fbebeb !important; }
